...
from fsim_training import fsim_training_loop as training_loop
...
@click.option('--divergence',     help='divergence you select to calculate the loss',    type=str,default='Forward-KL', show_default=True)
@click.option('--variance_reduction',       help='Whether to use variance reduction during training', metavar='BOOL',   type=bool, default=False, show_default=True)
@click.option('--ddo',       help='Whether use ddo as the teacher model', metavar='BOOL',   type=bool, default=False, show_default=True)
...
paper "Adversarial Score Identity Distillation: Rapidly Surpassing the Teacher in One Step".
    ...
    c.network_kwargs.class_name = 'fsim_training.fsim_networks.EDMPrecond_EncoderDecoder'
    #c.loss_kwargs.class_name = 'training.sid_loss.SID_EDMLoss'
    c.loss_kwargs.class_name ='fsim_training.fsim_loss.FSIM_EDMLoss'
    ...
    c.update(loss_scaling_D=opts.lsd, loss_scaling_G_gan=opts.lsg_gan,use_gan=opts.use_gan, divergence=opts.divergence, variance_reduction=opts.variance_reduction)    
    ...
    desc = f'{dataset_name:s}-{algorithm_str:s}-{cond_str:s}-{opts.arch:s}-{opts.precond:s}-glr{opts.glr}-lr{opts.lr}-ls{opts.ls}_lsg{opts.lsg}_lsd{opts.lsd}_lsg_gan{opts.lsg_gan}-initsigma{opts.init_sigma}-gpus{dist.get_world_size():d}-batch{c.batch_size:d}-tmax{c.tmax:d}-{dtype_str:s}'
    if opts.sid_model is not None:
    ...
    if opts.resume is not None:
        ...
        print(f"match: {match}")
        print(f"os.path.basename(opts.resume): {os.path.basename(opts.resume)}")
