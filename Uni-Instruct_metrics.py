...
def calculate_metric(metric, G, init_sigma, dataset_kwargs, num_gpus, rank, device,data_stat,detector_url,detector_dino):
    return metric_main.calc_metric(metric=metric, G=G, init_sigma=init_sigma,
        dataset_kwargs=dataset_kwargs, num_gpus=num_gpus, rank=rank, device=device,data_stat=data_stat,detector_url=detector_url, detector_dino=detector_dino)
...
@click.option('--detector_dino',          help='fd dino', metavar='BOOL',              type=bool, default=False, show_default=True)
...
def main(**kwargs):
    ...
    detector_dino = opts.detector_dino
    ...
    for i in range(10):    
        for metric in metrics:
            ...
            result_dict = calculate_metric(metric=metric, G=G_ema, init_sigma=init_sigma,
                dataset_kwargs=dataset_kwargs, num_gpus=dist.get_world_size(), rank=dist.get_rank(), device=device,data_stat=data_stat,detector_url=detector_url, detector_dino=detector_dino)
            if dist.get_rank() == 0:
