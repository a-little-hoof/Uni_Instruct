...
def fid50k_full(opts):
    ...
    if detector_dino is None:
        fid = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=50000,batch_size=opts.batch_size,batch_gen=opts.batch_gen)
        return dict(fid50k_full=fid)
    else:
        fid,fid_dino = frechet_inception_distance.compute_fid(opts, max_real=None, num_gen=50000,batch_size=opts.batch_size,batch_gen=opts.batch_gen, detector_dino=detector_dino)
        return dict(fid50k_full=fid,fid_dino=fid_dino)

@register_metric
def sfid50k_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    sfid = frechet_inception_distance.compute_sfid(opts, max_real=None, num_gen=50000)
    return dict(sfid50k_full=sfid)
