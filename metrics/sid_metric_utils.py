...
class MetricOptions:
    def __init__(self, G=None, init_sigma=None, G_kwargs={}, dataset_kwargs={}, data_stat=None, detector_url=None,num_gpus=1, rank=0, local_rank=0, device=None, progress=None, cache=False, batch_size=64, batch_gen=1, detector_dino=None):
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, detector_dino=None, **stats_kwargs):
    ...
    print(detector_dino, "detector_dino")
    
    if detector_dino is not None:
        detector_dino = DINOv2Detector()
    ...
    for images, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        ...
        if detector_dino is not None:
            ...
            with torch.no_grad():
                features_dino = detector_dino(x.to(opts.device))
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, detector_dino=None, **stats_kwargs):
...
def compute_sfid_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, detector_dino=None, **stats_kwargs):
    #compute the dataset feature stats using the training data
    dataset = dnnlib.util.construct_class_by_name(**opts.dataset_kwargs)
    if data_loader_kwargs is None:
        data_loader_kwargs = dict(pin_memory=True, num_workers=3, prefetch_factor=2)
    ...
    progress = opts.progress.sub(tag='dataset features', num_items=num_items, rel_lo=rel_lo, rel_hi=rel_hi)
    # detector = get_feature_detector(url=detector_url, device=opts.device, num_gpus=opts.num_gpus, rank=opts.rank, verbose=progress.verbose)
    detector = InceptionEncoder(sinception=True)
    detector.to(opts.device)
    ...
    for images, _labels in torch.utils.data.DataLoader(dataset=dataset, sampler=item_subset, batch_size=batch_size, **data_loader_kwargs):
        if images.shape[1] == 1:
            images = images.repeat([1, 3, 1, 1])
        features = detector(detector.transform(images.to(opts.device)))[0]
...
def compute_sfid_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, detector_dino=None, **stats_kwargs):
    if batch_gen is None:
        batch_gen = min(batch_size, 4)
    assert batch_size % batch_gen == 0
