...
def compute_fid(opts, max_real, num_gen,batch_size=64,batch_gen=1,detector_dino=None):
    # Direct TorchScript translation of http://download.tensorflow.org/models/image/imagenet/inception-2015-12-05.tgz
    #detector_url = 'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada-pytorch/pretrained/metrics/inception-2015-12-05.pt'
    if opts.detector_url is None:
        detector_url = 'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada-pytorch/pretrained/metrics/inception-2015-12-05.pt'
    ...
    if detector_dino is None:
        ...
        if opts.data_stat is not None:
            ...
            opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
            rel_lo=0, rel_hi=0, capture_mean_cov=True, max_items=max_real, detector_dino=detector_dino)
        else:
            # import ipdb; ipdb.set_trace()
            fid_stats, dino_stats = metric_utils.compute_feature_stats_for_dataset(
            opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
            rel_lo=0, rel_hi=0, capture_mean_cov=True, max_items=max_real, detector_dino=detector_dino)
            mu_real, sigma_real = fid_stats.get_mean_cov()
            mu_real_dino, sigma_real_dino = dino_stats.get_mean_cov()
        ...
        fid_stats, dino_stats = metric_utils.compute_feature_stats_for_generator(
            opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
            rel_lo=0, rel_hi=1, batch_size=batch_size,batch_gen=batch_gen,capture_mean_cov=True, max_items=num_gen, detector_dino=detector_dino)
...
def compute_sfid(opts, max_real, num_gen):
    # Direct TorchScript translation of http://download.tensorflow.org/models/image/imagenet/inception-2015-12-05.tgz
    #detector_url = 'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada-pytorch/pretrained/metrics/inception-2015-12-05.pt'
    if opts.detector_url is None:
        detector_url = 'https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada-pytorch/pretrained/metrics/inception-2015-12-05.pt'
    ...
    if opts.data_stat is not None:
        ...
        mu_real, sigma_real = metric_utils.compute_sfid_feature_stats_for_dataset(
        opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
        rel_lo=0, rel_hi=0, capture_mean_cov=True, max_items=max_real).get_mean_cov()

    # The generator component remains unchanged from the original code.
    mu_gen, sigma_gen = metric_utils.compute_sfid_feature_stats_for_generator(
        opts=opts, detector_url=detector_url, detector_kwargs=detector_kwargs,
        rel_lo=0, rel_hi=1, capture_mean_cov=True, max_items=num_gen).get_mean_cov()
    ...
    m = np.square(mu_gen - mu_real).sum()
    s, _ = scipy.linalg.sqrtm(np.dot(sigma_gen, sigma_real), disp=False) # pylint: disable=no-member
    sfid = np.real(m + np.trace(sigma_gen + sigma_real - s * 2))
    return float(sfid)
