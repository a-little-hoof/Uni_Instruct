...
# Modified by Yifei Wang for Uni-Instruct project.
...
def training_loop(
    ...
    if ddo == True:
        edm2_img64_ddo_url = "https://huggingface.co/nvidia/DirectDiscriminativeOptimization/resolve/main/edm2-img64-s-ddo.pkl?download=true"
        filename = "edm2-img64-s-ddo.pkl"
        if not os.path.exists(filename):
            urllib.request.urlretrieve(edm2_img64_ddo_url, filename)
        
        # 加载模型或数据
        true_score = torch.load(filename, map_location='cpu')  # 你也可以设置 map_location='cuda'
        true_score.eval().requires_grad_(False).to(device)
    ...
    if resume_pkl is not None:
        ...
        if resume_training is not None:
            ...
            # Setup optimizer.
            misc.copy_params_and_buffers(src_module=data['ema'], dst_module=fake_score, require_all=True)
    ...
    if dist.get_rank() == 0:
        save_data(data=data, fname=os.path.join(run_dir, f'network-snapshot-{cur_nimg//1000:06d}.pkl'))
    ...
    while True:        
            ...
            with misc.ddp_sync(G_ddp, False):
                ...
                with misc.ddp_sync(fake_score_ddp, (round_idx == num_accumulation_rounds - 1)):
                    ...
                    if use_gan is False:
                        ...
                        loss,loss_D = loss_fn.fakescore_discriminator_share_encoder_loss(fake_score=fake_score_ddp, images=images, labels=labels, augment_pipe=augment_pipe, real_images=real_images,true_score=true_score) 
                        loss=loss.sum().mul(loss_scaling / batch_gpu_total)
        ...
        with misc.ddp_sync(fake_score_ddp, False):
            for round_idx in range(num_accumulation_rounds):
                ...
                with misc.ddp_sync(G_ddp, (round_idx == num_accumulation_rounds - 1)):
                    ...
                    if use_gan is False:    
                        loss = loss_fn.generator_loss(true_score=true_score, fake_score=fake_score_ddp, images=images, labels=labels, augment_pipe=None,tmax=tmax)
                        lossG_gan_print = 0
                    else:
                        loss,loss_gan, y_real, y_fake, y = loss_fn.generator_share_encoder_loss(true_score=true_score, fake_score=fake_score_ddp, images=images, labels=labels, augment_pipe=None,tmax=tmax)
                        ...
                        # diff_instruct_loss = loss_gan.clone()
                    ...
                    if use_gan is True and cur_nimg>200*1000:
                        ...
                        if divergence == "Forward-KL":
                            with torch.no_grad():
                                density_ratio = -loss_gan/(1-loss_gan)
                                if variance_reduction == 1:
                                    sum_ratio_batch = torch.sum(density_ratio, dim=0, keepdim=True)
                                    density_ratio = density_ratio / (sum_ratio_batch + 1e-8)
                            # (loss*density_ratio+loss_gan).backward()
                            # dist.print0(loss.shape,"loss shape")
                            # dist.print0(density_ratio.shape,"density shape")
                            loss = (loss*density_ratio).sum().mul(loss_scaling_G / batch_gpu_total)
                            loss_gan = (loss_gan).sum().mul(loss_scaling_G_gan / batch_gpu_total)
                            diff_instruct_loss = loss_gan.clone()
                            lossG_gan_print = loss_gan.item()
                            (loss+loss_gan).backward()
                            ...
                            with torch.no_grad():
                                ...
                                if variance_reduction == 1:
                                    sum_ratio_batch = torch.sum(density_ratio, dim=0, keepdim=True)
                                    density_ratio = density_ratio / (sum_ratio_batch + 1e-8)
                            f_loss = (loss*density_ratio).sum().mul(loss_scaling_G / batch_gpu_total)
                            r_loss = loss.sum().mul(loss_scaling_G / batch_gpu_total)
                            loss_gan = (loss_gan).sum().mul(loss_scaling_G_gan / batch_gpu_total)
                            loss = 0.4*f_loss + 0.6*r_loss
                            lossG_gan_print = loss_gan.item()
                            diff_instruct_loss = loss_gan.clone()
                            (0.4*f_loss + 0.6*r_loss + loss_gan).backward()
                        elif divergence == "Jensen-Shannon":
                            with torch.no_grad():
                                density_ratio = loss_gan/(1-loss_gan)
                                # weighting for sim
                                w_sim = density_ratio/(density_ratio+1)
                                # weighting for diff-instruct
                                w_diff_instruct = 2*density_ratio**2/((density_ratio+1)**3)*(y_real-y_fake)*torch.norm(y_real-y_fake, p=2)
                                # weighting for FGM loss
                                w_FGM = density_ratio**2/(density_ratio+1)**2
                                ###FIXME variance reduction

                            diff_instruct_loss = (w_diff_instruct*y).sum().mul(loss_scaling_G / batch_gpu_total)
                            # optional FGM loss
                            FGM_loss = (w_FGM*torch.norm(y_real-y_fake, p=2)).sum().mul(loss_scaling_G / batch_gpu_total)
                            # (loss*w_sim + diff_instruct_loss + FGM_loss).backward()
                            (loss*w_sim + diff_instruct_loss).backward()
                        elif divergence == "Chi-Square":
                            with torch.no_grad():
                                density_ratio = loss_gan/(1-loss_gan)
                                w_diff_instruct = (y_real-y_fake)*torch.norm(y_real-y_fake, p=2)
                                ###FIXME variance reduction

                            diff_instruct_loss = (y*w_diff_instruct).sum().mul(loss_scaling_G / batch_gpu_total)
                            # optional FGM loss
                            FGM_loss = torch.norm(y_real-y_fake, p=2).sum().mul(loss_scaling_G / batch_gpu_total)
                            # (density_ratio**2*(2*loss-4*diff_instruct_loss-2*FGM_loss)).backward()
                            (density_ratio**2*(2*loss-4*diff_instruct_loss)+loss_gan).backward()
                            ...
                            lossG_gan_print = loss_gan.item()
                            (loss).backward()
                        elif divergence == "f-distill-Reverse-KL":
                            with torch.no_grad():
                                w_diff_instruct = -(y_real-y_fake)
                            diff_instruct_loss = (y*w_diff_instruct).sum().mul(loss_scaling_G / batch_gpu_total)
                            ((diff_instruct_loss)+loss_gan).backward()
                        elif divergence == "f-distill-Forward-KL":
                            with torch.no_grad():            
                                if variance_reduction == 0:
                                    density_ratio = loss_gan/(1-loss_gan)
                                elif variance_reduction == 1:
                                    density_ratio = torch.clamp(torch.exp(loss_gan), min=1e-3, max=1e3)
                                    sum_ratio_batch = torch.sum(density_ratio, dim=0, keepdim=True)
                                    density_ratio = density_ratio / (sum_ratio_batch + 1e-6)
                                w_diff_instruct = (y_real-y_fake)
                            diff_instruct_loss = (y*w_diff_instruct).sum().mul(loss_scaling_G / batch_gpu_total)
                            (density_ratio*(diff_instruct_loss)+loss_gan).backward()
                        elif divergence == "f-distill-Jensen-Shannon":
                            with torch.no_grad():
                                if variance_reduction == 0:
                                    density_ratio = loss_gan/(1-loss_gan)
                                    h_x = -density_ratio/(density_ratio+1)
                                elif variance_reduction == 1:
                                    density_ratio = torch.clamp(torch.exp(loss_gan), min=1e-3, max=1e3)
                                    sum_ratio_batch = torch.sum(density_ratio, dim=0, keepdim=True)
                                    density_ratio = density_ratio / (sum_ratio_batch + 1e-6)
                                    h_x = density_ratio/(density_ratio+1)
                                    sum_hx_batch = torch.sum(h_x, dim=0, keepdim=True)
                                    h_x = h_x / (sum_hx_batch + 1e-6)

                                w_diff_instruct = (y_real-y_fake)
                            diff_instruct_loss = (y*w_diff_instruct).sum().mul(loss_scaling_G / batch_gpu_total)
                            ((h_x)*(diff_instruct_loss)+loss_gan).backward()
        ...
        loss_diff_instruct_print = diff_instruct_loss.item()
        training_stats.report('Diff_Instruct_Loss/loss', loss_diff_instruct_print)
        ...
        fields += [f"loss_diff_instruct {training_stats.report0('Diff_Instruct_Loss/loss', loss_diff_instruct_print):<6.2f}"]
            ...
            if dist.get_rank() == 0:  # Check if this is the master process
                if save_best_and_last is False:
                    ...
                    if current_fid < best_fid:  # Check if the current model is better
                        ...
                        new_best_pkl_filename = os.path.join(run_dir, f'network-snapshot-{cur_nimg//1000:06d}.pkl')
